...
def find_list_of_files(data_dir):
    ...
    for f in listdir(data_dir):
        ...
        if isfile(file_path) and not ("~" in f) and not (".gitignore" == f)\
                and not ("meta_data" in f) and not ("test_res" in f)\
                and not ("actor" in f) and not (".ckpt" in f)\
                and not ("checkpoint" in f)\
                and not (".pb" in f):
...
def sort_files(file_list, mode):
    file_time_stamp_dic = list(
        map(lambda x: (get_time_stamp(x), x), file_list)
    )
    # sort(reverse = (mode == 'reverse'))
...
def find_all_weight_files(algo, root_dir):
    ...
    for parent_file_names in dir_files:
        ...
        if algo == "DQN":
            ...
            check_point_files += list(
                map(
                    lambda x:
                    {
                        "actor": join(
                            algo_data_dir_path, dir_name, x
                        ),
                        "critic": join(
                            algo_data_dir_path, dir_name,
...
class Backup():
    ...
    def make_dir_to_copy_to(self, dir_path):
        ...
        if (len(all_backedup_dirs_path) == 0):
            dir_to_copy_to = join(
                dir_to_look_at, settings.backup_folder_name_style
            )
        else:
            dir_to_copy_to = incr_file_name(
                find_file_or_dir(
                    all_backedup_dirs_path, 'newest'
                )
            )
