            ...
            with tf.variable_scope("action_value"):
                if feature_extraction == "cnn":
                    ...
                    for layer_size in layers:
                        ...
                        action_out = tf_layers.Dense(
                            action_out, num_outputs=layer_size, activation=None
                        )
                        if layer_norm:
                            # action_out = tf_layers.layer_norm(action_out, center=True, scale=True)
                            action_out = tf_layers.LayerNormalization(
                                action_out, axis=-1)
                        action_out = act_fun(action_out)

                # action_scores = tf_layers.fully_connected(action_out, num_outputs=self.n_actions, activation_fn=None)
                action_scores = tf_layers.Dense(
                    action_out, num_outputs=self.n_actions,
                    activation=None
                )
            ...
            if self.dueling:
                with tf.variable_scope("state_value"):
                    ...
                    for layer_size in layers:
                        ...
                        state_out = tf_layers.Dense(
                            state_out, num_outputs=layer_size,
                            activation=None
                        )
                        if layer_norm:
                            # state_out = tf_layers.layer_norm(state_out, center=True, scale=True)
                            state_out = tf_layers.LayerNormalization(
                                action_out, axis=-1)
                        state_out = act_fun(state_out)
                    state_score = tf_layers.Dense(
                        state_out, num_outputs=1,
                        activation=None
                    )
